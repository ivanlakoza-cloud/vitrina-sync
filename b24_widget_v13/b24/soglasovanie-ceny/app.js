...
// v13 — stricter isolation + cache-busting already in HTML
...
function validateAll(){
  ...
  qs('#b24app #submitBtn').disabled = !ok;
  return ok;
}
...
async function detectDealId(){
  ...
  for(const k of ['ID','id','deal_id','DEAL_ID','entityId']){
...
async function ensureBX(){
  ...
  if(!window.BX24){ showAlert('error','BX24 недоступен. Откройте виджет внутри карточки сделки.'); throw new Error('BX24 not available'); }
  try{
    await new Promise((resolve)=>{ let resolved=false; BX24.init(function(){resolved=true; resolve();}); setTimeout(()=>{ if(!resolved) resolve(); }, 1200); });
  }catch(e){ showAlert('error','Ошибка инициализации BX24: '+e.message); throw e; }
}
...
async function onSubmit(){
  ...
  const btn = qs('#b24app #submitBtn');
  btn.disabled = true; btn.textContent = 'Отправляем…';
  try{
    const changed = buildDiff();
    if(Object.keys(changed).length>0){ await callBX('crm.deal.update', {id: DEAL_ID, fields: changed}); }
