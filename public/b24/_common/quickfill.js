/* QuickFill core (Bitrix24) — v1.3 (debug + robust ID detect)
 - Абсолютно безопасные правки: добавлен fallback получения ID сделки из document.referrer
 - Больше логов в console и видимый статус при залипании BX24.init
*/
(function(global){
  const log = (...a)=>{ try{ console.log('[QuickFill]', ...a); }catch(e){} };
  const QuickFill = {
    boot(cfg){
      this.cfg = Object.assign({ENTITY_TYPE:'DEAL',REQUIRED_CODES:[],BP_TEMPLATE_ID:null,TITLE:'Заполнить обязательные поля',BP_PARAMETERS:{}}, cfg||{});
    ...
    // --- ключевой блок: определение ID сделки ---
    resolveEntityId(cb){
      ...
      BX24.placement.info((data)=>{
        ...
        if(!id){
          // 2) document.referrer (CRM URL вида /crm/deal/details/12345/)
    ...
    render(){
      ...
      f.addEventListener('submit', e=>{ e.preventDefault(); this.clearMsgs(); const fields={};
        m.forEach(code=>{ const el=f.querySelector('#fld_'+code); if(!el) return; const v=el.value; if(v==='') return; fields[code]=v; });
    ...
    init(){
      ...
      // Страховка на случай зависания init
      let initWatch = setTimeout(()=>{
        this.msg('Нет ответа от BX24.init(). Проверьте, что приложение открыто из плейсмента/вкладки и не заблочены куки/скрипты.','error');
      }, 5000);
