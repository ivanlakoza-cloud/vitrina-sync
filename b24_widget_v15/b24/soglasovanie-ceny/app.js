...
// v15 build with visible version badge
...
async function loadDeal(){
  // Show build + rev badges immediately
  const bb = qs('#buildBadge'); if(bb) bb.textContent = `Проверка ${BUILD.num}`;
  const rb = qs('#revBadge'); if(rb) rb.textContent = QUERY.rev ? `rev=${QUERY.rev}` : 'rev=?';
  document.title = `Проверка полей сделки [${BUILD.tag}]`;
