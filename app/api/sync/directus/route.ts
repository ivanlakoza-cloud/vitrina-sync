...
function assertAuth(req: NextRequest): boolean {
  ...
  const secret = getEnv('DIRECTUS_WEBHOOK_SECRET').trim()
...
async function propertyTagsForExisting(supabase: SupabaseClient, collection: Collection, ids: string[]): Promise<string[]> {
  const tags = new Set<string>()
  if (collection === 'properties') {
    const { data, error } = await supabase.from('properties').select('id, external_id').in('id', ids)
    if (error) throw error
    for (const r of data || []) {
      if (r.id) tags.add(String(r.id))
      if (r.external_id) tags.add(String(r.external_id))
    }
...
async function enrichPropertyExternalIds(supabase: SupabaseClient, propertyTags: string[]): Promise<string[]> {
  const out = new Set<string>(propertyTags)
  const ids = propertyTags.filter((t) => /^[0-9a-f-]{10,}$/i.test(t))
  if (ids.length) {
