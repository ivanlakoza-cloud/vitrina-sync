...
(() => {
  ...
  // ---- Config mapping (под капотом UF_*) ----
  const CONFIG = {
    fields: {
      area: 'UF_CRM_AREA_M2',
  ...
  async function detectDealId(){
    ...
    if(window.BX24 && BX24.placement){
      try{
        ...
        if(info?.options?.ID) return {id:String(info.options.ID), source:'placement'};
      }catch{}
    }
    // 2) query
    const q = getQuery();
  ...
  const lastSaved = Object.create(null); // по полям, что уже записали в Б24
  ...
  // ---- Автосохранение по blur ----
  async function autosaveField(fieldId){
    try{
      const el = $("#"+fieldId);
      if(!el) return;
      const value = String(el.value ?? "").trim();
      const uf = CONFIG.fields[fieldId];
      if(!uf) return;
      // если не изменилось с последнего успешного сохранения — ничего не шлём
  ...
  // ---- Сабмит (финальный): обновить оставшиеся изменения и запустить БП 209 ----
  async function onSubmit(e){
    e?.preventDefault();
    if(!validateAll()){
      status("Заполните обязательные поля.");
      return;
    }
    $("#btnSend").disabled = true;
    status("Отправляем…");

    // собрать diff относительно lastSaved (на случай, если пользователь не покинул поле)
    const changed = {};
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    try{
      ...
      // зафиксировать lastSaved начальными значениями
      ...
      // навесить autosave на blur и обновление валидности
      for(const key of Object.keys(CONFIG.fields)){
        const el = $("#"+key);
        if(!el) continue;
